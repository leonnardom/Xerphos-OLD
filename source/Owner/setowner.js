...
database.Users.findOne({_id: member.id}, function (erro, usuario) {
            ...
            if (usuario) {
              if (usuario.owner) {
                usuario.owner = false
                usuario.save()
                message.channel.send(`<a:okay:512392301037748251> | ${message.author}, **${member.tag}** não é mais **\`Dono\`**`)
              } else {
                usuario.owner = true
                usuario.save()
                message.channel.send(`<a:okay:512392301037748251> | ${message.author}, **${member.tag}** agora é **\`Dono\`**`)
              }
            } else {
