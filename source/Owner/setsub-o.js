...
console.log(`comando setsub-o ${message.guild.name} ${message.guild.id} ${message.author.tag}`)
...
database.Users.findOne({_id: member.id}, function (erro, usuario) {
            ...
            if (usuario) {
              if (usuario.subowner) {
                ...
                usuario.save()
                message.channel.send(`<a:okay:512392301037748251> | ${message.author}, **${member.tag}** não é mais **\`Sub-Dono\`**`)
              } else {
                usuario.subowner = true
                usuario.save()
                message.channel.send(`<a:okay:512392301037748251> | ${message.author}, ${member.tag}** agora é **\`Sub-Dono\`**`)
              }
            } else {
